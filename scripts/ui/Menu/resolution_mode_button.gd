...
func _ready() -> void:
	...
	# Permite conectar o sinal pelo Inspetor ou pelo código sem correr o risco de conectar duas vezes.
...
func _add_resolution_items() -> void:
	...
	for resolution in DISPLAY_RESOLUTIONS:
		...
		option_button.add_item(_resolution_to_text(resolution))
		option_button.set_item_metadata(index, resolution)
...
func _select_current_resolution() -> void:
	...
	# Caso a resolução atual não exista na lista, adicionamos ela temporariamente para o menu mostrar corretamente a resolução usada no momento.
	if matching_index == -1:
...
func _on_resolution_selected(index: int) -> void:
	...
	# O tamanho físico da janela só vai ser alterado quando estiver em Janela.
	# Em fullscreen, o Godot utiliza o tamanho do monitor.
	...
	# Espera 1 frame pro sistema operacional atualizar o tamanho real da janela antes de centralizar.
...
func _center_window() -> void:
	# Descobre em qual tela a janela tá.
	var screen := DisplayServer.window_get_current_screen()

	# Pega só a área utilizável da tela.
	# Isso evita colocar a janela atrás da barra de tarefas.
	var usable_rect := DisplayServer.screen_get_usable_rect(screen)

	# Inclui também bordas e barra de título da janela.
